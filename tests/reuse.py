...
def fresh():
    core._approx_state.clear(); core._cascade_state.clear(); core._tiles1_slow.clear()
...
for name, d, k, exact in [("fused  d=8   k=16", 8, 16, True), ("tiles1 d=48  k=64", 48, 64, True),
                          ...
                          ("approx d=64  k=32", 64, 32, False)]:
    fresh()
    A = rng.standard_normal((n, d)).astype(np.float32)
    m = KMeans(n_clusters=k, max_iter=3, random_state=0, exact=exact).fit(A)
    ...
    m2 = KMeans(n_clusters=k, max_iter=3, random_state=0, exact=exact).fit(B)
    fresh()
    m3 = KMeans(n_clusters=k, max_iter=3, random_state=0, exact=exact).fit(B.copy())
