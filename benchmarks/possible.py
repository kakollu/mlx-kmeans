...
All rates from benchmarks/MACHINE-PROFILE.md (September 2026). Run: .venv/bin/python benchmarks/possible.py
"""
...
VISITED = 0.02          # fraction of the n x k distances a converged-regime iteration still evaluates with per-centre bounds (measured 0.3-3%)
...
for name, (n, d, k, measured) in SUITE.items():
    ...
    own = 2.0 * n * d / SCALAR
    conv32 = max(read, bt32, VISITED * gemm / SIMD + own)
    conv16 = max(read, bt16, VISITED * gemm / SIMD + own)
